...
def load_data(database_filepath):
    """
    函数作用：提取数据DisasterResponse中数据；
    输入变量：数据库文件路径
    输出变量：自变量X，因变量Y，因变量Y数据中所含的字段名称列表category_names
...
def tokenize(text):
    """
    函数作用：将文本进行所有大写字母转为小写字母，并按照停止词进行分词；
    输入变量：文本变量text
    输出变量：进行分词后的变量tokens
...
def evaluate_model(model, X_test, Y_test, category_names):
    """
    函数作用：估模型，评价维度：F1值，精确度，召回率；
    输入变量：模型，测试值X，测试值Y，Y的分类名称
    输出变量：无
